...
UnitTest.test('ResizeTest', () => {
  ...
  const testAdjustWidth = (msg: string, expectedWidth: number, expectedColumnWidths: number[], table: Element<HTMLTableElement>, step: number, index: number, direction: ResizeDirection, columnSizing: TableSize.ColumnSizing) => {
  ...
  const testInnerColumnResizing = () => {
    Arr.each([ 'default', 'static' ] as TableSize.ColumnSizing[], (mode) => {
  ...
  const testLastColumnResizing = () => {
    Arr.each([ 'static', 'resizetable' ] as TableSize.ColumnSizing[], (mode) => {
